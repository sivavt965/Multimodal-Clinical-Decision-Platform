...
SUPPORT_DEVICES_IDX = 7

if not os.path.exists(local_img_path):
    print("Could not find the local image.")
    sys.exit(1)

# ---- 2) Load Image & Apply Masking ----
img_pil = Image.open(local_img_path).convert("RGB")
img_np = np.array(img_pil, dtype=np.uint8)

# ABLATION: Mask out the top 20% of the image where the text/markers are
h, w = img_np.shape[:2]
mask_h = int(h * 0.18)  # top 18%
img_masked = img_np.copy()
img_masked[0:mask_h, :] = 0  # Black out the text and markers
...
x = tf(image=img_masked)["image"].unsqueeze(0).float()
...
# ABLATION: Target an earlier layer (denseblock3) for HIGHER RESOLUTION CAM
# denseblock4 is 16x16, denseblock3 should be 32x32
target_layer = model.features.denseblock3
grad_cam = SimpleGradCAM(model, target_layer)

print("Running Higher-Res Grad-CAM on MASKED image...")
cam_map, probs = grad_cam(x, class_idx=SUPPORT_DEVICES_IDX)

pred_prob = probs[0, SUPPORT_DEVICES_IDX]
print(f"Probability for Support Devices (MASKED): {pred_prob:.4f}")

# ---- 5) Resize & Overlay Heatmap ----
cam_resized = cv2.resize(cam_map, (img_masked.shape[1], img_masked.shape[0]))
heatmap = cv2.applyColorMap(np.uint8(255 * cam_resized), cv2.COLORMAP_JET)
heatmap = cv2.cvtColor(heatmap, cv2.COLOR_BGR2RGB)

overlay = np.uint8(0.4 * heatmap + 0.6 * img_masked)

# ---- 6) Save Result ----
out_path = r"b:\mimic_project_full_backup\mimic_project\gradcam_support_devices_masked.jpg"
fig, ax = plt.subplots(1, 3, figsize=(15, 5))
ax[0].imshow(img_masked)
ax[0].set_title("Masked Image (Corners Blacked Out)")
ax[0].axis('off')
...
print(f"Saved MASKED Grad-CAM visualization to: {out_path}")
